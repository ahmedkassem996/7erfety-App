//
//  PreviewPhotoContainnerView.swift
//  InstegramFirebase
//
//  Created by AHMED on 6/29/1398 AP.
...
class PreviewPhotoContainnerView: UIView {
  ...
  let saveButton: UIButton = {
    let button = UIButton(type: .system)
    button.setImage(UIImage(named: "saveButton")?.withRenderingMode(.alwaysOriginal), for: .normal)
  ...
  @objc func handleSave(){
    ...
    library.performChanges({
      ...
      if let err = err{
        print("Failed to save image photo library:", err)
        return
      }
